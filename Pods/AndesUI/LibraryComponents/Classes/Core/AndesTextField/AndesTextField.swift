...
@objc public class AndesTextField: UIView, AndesAccessibleView {
    ...
    @IBInspectable public var helper: String? {
        didSet {
            helper = (helper == nil || helper == "") ? " ": helper
            self.updateContentView()
        }
    ...
    @objc public var helperLinks: AndesBodyLinks? {
        didSet {
            self.updateContentView()
        }
    }
    ...
    @objc public var helperBolds: AndesBodyBolds? {
        didSet {
            self.updateContentView()
        }
    }
    ...
    @objc public init(state: AndesTextInputState, label: String?, helper: String?, counter: UInt16, placeholder: String?, helperLinks: AndesBodyLinks? = nil, helperBolds: AndesBodyBolds? = nil) {
    ...
    private func setup() {
        ...
        accessibilityManager = AndesTextFieldAccessibilityManager(view: self)
    }
    ...
    internal func onTapCheckbox(with status: AndesCheckboxStatus) {
        self.delegate?.andesTextFieldDidTapRightCheckbox?(self, with: status)
    }
}
...
extension AndesTextField {
    ...
    @objc override public var isFirstResponder: Bool {
