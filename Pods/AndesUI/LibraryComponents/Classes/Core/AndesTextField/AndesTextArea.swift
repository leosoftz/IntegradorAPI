...
@objc public class AndesTextArea: UIView {
    ...
    @IBInspectable public var helper: String? {
        didSet {
            helper = (helper=="" ||  helper==nil) ? " ": helper // workaround because helper label set the height of text counter
            self.updateContentView()
        }
    }
    ...
    @objc public init(state: AndesTextInputState, label: String?, helper: String?, counter: UInt16, placeholder: String?, maxLines: UInt16 = 0, helperLinks: AndesBodyLinks? = nil, helperBolds: AndesBodyBolds? = nil) {
    ...
    private func setup() {
        ...
        accessibilityManager = AndesTextAreaAccessibilityManager(view: self)
    }
